...
\begin{itemize}
    \item[$i, j \in \mathcal{N}$] Buses
    \item[$(i,j) \in \mathcal{E}$] Branches (transmission lines)
\end{itemize}

\subsection*{Variables}
\begin{itemize}
    \item[$\boldsymbol{p}$] Generator active power dispatch vector
    \item[$\boldsymbol{r}$] Generator reserve dispatch vector
    \item[$\theta$] Bus voltage angle vector
\end{itemize}

\subsection*{Parameters}
\begin{itemize}
    \item[$\boldsymbol{d}$] Active power demand vector
    \item[$c(\cdot)$] Production cost function
    \item[$\overline{\boldsymbol{p}}$] Maximum generator output vector
    \item[$\overline{\boldsymbol{r}}$] Maximum generator reserve vector
    \item[$R$] Minimum reserve requirement
    \item[$\boldsymbol{B}$] Bus susceptance matrix
    \item[$x_{ij}$] Reactance of branch $(i,j)$
    \item[$\overline{P}_{ij}$] Thermal limit of branch $(i,j)$
    \item[$\boldsymbol{B}^{\dagger}$] Moore-Penrose pseudo-inverse of $\boldsymbol{B}$
    \item[$\mathbf{e}$] Vector of ones
    \item[$M_{pb}$] Power balance penalty multiplier
    \item[$M_{res}$] Reserve penalty multiplier
    \item[$M_{th}$] Thermal limit penalty multiplier
    \item[$M_{dc}$] DC power flow penalty multiplier
\end{itemize}

\end{multicols}
\section{Introduction}
The paper~\cite{Chen2023_E2ELR} proposes an End-to-End Learning and Repair (E2ELR) architecture for training optimization proxies for economic dispatch problems. That is, at the end of the network, there are repair layers that ensure power balance and reserve. As stated in the paper~\cite{Chen2023_E2ELR}, if there is a feasible solution, the repair layers will output a feasible solution, and if there is no feasible solution, the repair layers will minimize in some sense the violation of the constraints. In this work, we merge the ED problem with the DC-OPF problem~\cite{Stott2009_DCPF}, and we propose to add a layer that will ensure the satisfaction of the DC power flow equations.
...
\begin{enumerate}
    ...
    \item $\boldsymbol{B} \theta = \boldsymbol{p} - \boldsymbol{d}$ (DC power flow equations~\cite{Stott2009_DCPF})
\end{enumerate}
...
As seen in the paper~\cite{Chen2023_E2ELR}, conditions 2, 3, and 5 are equivalent to the condition $R \leq e^T \min(\overline{\boldsymbol{r}}, \overline{\boldsymbol{p}} - \boldsymbol{p})$, and removing $\boldsymbol{r}$ from the optimization variables.
